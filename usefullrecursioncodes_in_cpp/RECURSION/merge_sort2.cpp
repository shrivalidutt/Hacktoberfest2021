...
void merge(int arr[],int s,int e)
{
    ...
    int arr1[size1+1],arr2[size2+1];     //creating the two arrays with one size more bcz there we fill INT_MAX;

                                     //creating third pointer of main array which starts from s=0,so that we can fill it properly
    for(int i=0;i<size1;i++){        //here we are copying the two divided array into the new arr1 with its pointer i
        arr1[i]=arr[s+i];            
    }

                                     //here the third pointer starts from mid+1 bcz of arr2 start from the same
